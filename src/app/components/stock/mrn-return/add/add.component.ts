...
export class MrnReturnAddComponent {
  ...
  get controls(){
    return (<FormArray>this.mrnReturn.get('itemsForm')).controls
  }
  ...
  addItem(){
    (<FormArray>this.mrnReturn.get('itemsForm')).push(this.createItem());

  }

  calculateAmount(index: number) {
    const item = (<FormArray>this.mrnReturn.get('itemsForm')).at(index);
